...
def get_chain(llm_model, is_local = True):
    ...
    class Result(BaseModel):
        id: str = Field(description="a number or none that represents the id od the best answer")
        response: str = Field(description="the response to the question")
    parser = PydanticOutputParser(pydantic_object=Result)
    new_parser = OutputFixingParser.from_llm(parser=parser, llm=llm_model)


    category_chain = (
        ChatPromptTemplate.from_template(
            """你要針對問題區分類別，
            主題類別如下["政治","機敏","台水服務","其他"]
    ...
    def route(input):
        if "政治" in input["category"]:
            ...
            return prompt | llm_model | new_parser
            ...
            prompt = ChatPromptTemplate.from_messages(messages)
            return prompt | llm_model | new_parser
        elif "其他" in input["category"]:
            rag_prompt = HumanMessagePromptTemplate.from_template("""你只要回傳: {{"id": "其他","response": "我是台水智能客服您詢問的項目不在服務範圍內。"}}""")
            messages = [SystemMessage(content="你現在是台水智能客服"), rag_prompt]
            prompt = ChatPromptTemplate.from_messages(messages)
            return prompt | llm_model | new_parser
        else:
